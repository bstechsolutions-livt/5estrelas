...
- ✅ **Renovação** (`GestaoContratosRenovacaoController`): index, contratos para renovar, preparar, renovar (com checagem de divergência).
...
- Seeders: `GestaoContratosTiposSeeder` (índices/tipos) + `GestaoContratosDemoSeeder` (contratos, alvarás, equipamentos, renovações).
- ✅ Ctrl+K: contratos e alvarás na busca global.

## PENDENTE (próximas fatias)
- Controllers ainda NÃO portados: `GestaoContratosMedicaoController`, `GestaoContratosDashboardController` (dashboard-recorrentes).
- **Acoplado a Compras + Oracle** (não funciona sem o módulo de Compras): contratos recorrentes, entrada-nota, financeiro, medições, dashboard-recorrentes.
- Mobile dedicado: as telas são responsivas (rodam dentro do AppLayout), mas não têm versão `.mobile.vue` própria ainda.
- Cores: telas usam roxo/`dark:` hardcoded (Tailwind). Modo escuro já foi desligado globalmente; falta trocar o roxo pelas vars de tema white-label.
