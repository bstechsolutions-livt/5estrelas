...
- ✅ **Equipamentos** (`GestaoEquipamentosController`): dashboard, CRUD, tipos, ocorrências, tratativas, fotos, histórico de validade, export CSV.
- Seeders: `GestaoContratosTiposSeeder` (índices/tipos) + `GestaoContratosDemoSeeder` (contratos, alvarás, equipamentos).
- ✅ Ctrl+K: contratos, alvarás e equipamentos na busca global.
- Menu Contratos: Painel, Locação, Serviços, Alvarás, Equipamentos, Tipos de Equip.

## REMOVIDO (não se aplica ao 5 Estrelas — sem módulo de Compras)
Removido por completo (telas, controller, model, rotas, menu, seeder, tabela) para não deixar código morto/confuso:
- **Renovação** (dependia de contratos recorrentes vindos do Compras; cobertura já existe via editar contrato + reajuste).
- Cluster acoplado a Compras/Oracle: **medições, dashboard-recorrentes, entrada-nota, financeiro, avisos, relatórios** (telas) + `contratos-recorrentes.js` + models de medição.
- Se um dia houver módulo de Compras, recuperar do histórico do git.

## PENDENTE (acabamento)
- Mobile dedicado: telas rodam responsivas dentro do AppLayout, sem `.mobile.vue` própria.
- Cores: telas usam roxo/`dark:` hardcoded. Modo escuro já desligado globalmente; falta trocar o roxo pelas vars de tema white-label.
