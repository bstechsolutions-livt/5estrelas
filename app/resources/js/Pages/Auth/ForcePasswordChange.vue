...
<template>
    ...
    <div v-if="isMobile" :class="['min-h-screen flex flex-col px-5 pb-8', hasLoginBg ? 'login-bg' : '']" :style="bgStyle">
        <div class="flex-1 flex flex-col justify-center pt-12">
            ...
            <div class="space-y-3">
                <h1 class="text-xl font-semibold text-white drop-shadow">Defina sua senha</h1>
                <p class="text-sm text-white/80 mb-3">
                    Você entrou com a senha padrão. Crie uma nova senha para continuar.
                </p>
                <p v-if="email" class="text-xs text-white/60">{{ email }}</p>
    ...
    <div v-else :class="['min-h-screen flex items-center justify-center px-4', hasLoginBg ? 'login-bg' : '']" :style="bgStyle">
        <div class="w-full max-w-md">
            ...
            <form @submit.prevent="submit" class="bg-white rounded-2xl shadow-xl p-8 space-y-5">
                <div>
                    <h2 class="text-xl font-semibold text-gray-800 mb-1">Defina sua senha</h2>
                    <p class="text-sm text-gray-500">
                        Você entrou com a senha padrão. Crie uma nova senha para continuar.
                    </p>
                    <p v-if="email" class="text-xs text-gray-400 mt-2">{{ email }}</p>
                </div>
                ...
                <Button type="submit" label="Salvar e continuar" icon="pi pi-check" :loading="form.processing" class="w-full" />
            </form>
        </div>
    </div>
</template>
