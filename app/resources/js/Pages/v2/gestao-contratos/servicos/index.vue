...
onMounted(async () => {
  ...
  GestaoJs.contratos.value.filtros.tipo = "SERVICO"
  await Promise.all([GestaoJs.getContratos(), GestaoJs.getFiliais()])
})

function novoContrato() {
  router.visit("/pagina/gestao-contratos/servicos/novo")
}

function editarContrato(id) {
  router.visit(`/pagina/gestao-contratos/servicos/${id}`)
}
...
async function exportar() {
  ...
  try {
    await GestaoJs.exportarContratosServico()
  } finally {
    ...
    <div class="space-y-2 mb-6 mt-4">
      ...
      >
        <div>
          ...
          >
            Gerencie os contratos de prestação de serviços da empresa.
          </span>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <div class="flex items-center gap-3 flex-1 min-w-0">
          ...
          >
            <i
              class="pi pi-list !text-xl text-emerald-700 dark:text-emerald-400"
            ></i>
          </span>
          <div>
            <h2
            ...
            >
              Lista de contratos de serviço cadastrados
            </div>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm">
        ...
        >
          ...
          >
            <template #body="{ data }">
              <div class="max-w-[220px]">
                <div
                  class="flex items-center gap-1.5 text-sm font-medium text-gray-900 dark:text-gray-200 truncate"
                >
                  <i class="pi pi-user text-purple-500 shrink-0"></i>
                  <span class="truncate">{{ data.nome_locador || "-" }}</span>
                </div>
                <div
                  v-if="data.documento_locador"
                  class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400"
                >
                  <i class="pi pi-id-card text-gray-400 shrink-0"></i>
                  {{
                    formatarCnpj(data.documento_locador) ||
                    data.documento_locador
                  }}
                </div>
              </div>
            </template>
          ...
          >
            <template #body="{ data }">
              <div
                class="flex items-center gap-1.5 text-sm text-gray-900 dark:text-gray-200 truncate max-w-xs"
              >
                <i class="pi pi-file-edit text-cyan-500"></i>
