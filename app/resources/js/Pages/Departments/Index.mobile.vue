...
function confirmDelete() {
    ...
    if (selected.value) {
        router.delete(`/departamentos/${selected.value.id}`, { preserveScroll: true })
    }
}
</script>

<template>
    <AppLayoutMobile title="Departamentos">
        <div class="px-4 pt-3 pb-2">
            <InputText v-model="search" placeholder="Buscar departamento..." class="w-full" style="height: 44px" />
        </div>

        <div v-if="departments.data.length" class="px-4 space-y-2 pb-24">
            <button
                v-for="dept in departments.data"
                :key="dept.id"
                @click="openActions(dept)"
            ...
            >
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">{{ dept.users_count }} usuário(s)</p>
                </div>
                <Tag :value="dept.is_active ? 'Ativo' : 'Inativo'" :severity="dept.is_active ? 'success' : 'secondary'" />
            </button>
        </div>
