            ...
            <div class="p-4 sm:p-5 min-w-0">
                ...
                <div class="grid grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-4 text-sm">
                    <div>
                        ...
                        <p class="text-gray-800">{{ payable.empresa_nome || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Filial</p>
                        <p class="text-gray-800">{{ payable.filial_label || payable.filial_nome || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Natureza de gasto</p>
                        <p class="text-gray-800">{{ naturezaGasto || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Centro de custo</p>
                        <p class="text-gray-800 text-xs" :class="centroCusto && !payable.centro_custo_nome ? 'font-mono' : ''">{{ centroCusto || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Conta financeira</p>
                        <p class="text-gray-800 text-xs" :class="contaFinanceira && !payable.conta_financeira_nome ? 'font-mono' : ''">{{ contaFinanceira || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Departamento</p>
                        <p class="text-gray-800">{{ departamentoTitulo || '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">Emissão</p>
                        <p class="text-gray-800">{{ formatDate(payable.issue_date) }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500">CNPJ</p>
                        <p class="text-gray-800 font-mono text-xs">{{ payable.supplier_cnpj || '—' }}</p>
                    </div>
                    <div dusk="payable-lancado-por">
                        <p class="text-xs text-gray-500">Lançado por (Senior)</p>
