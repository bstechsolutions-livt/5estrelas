...
/**
 * Cliente SOAP read-only do cadastro de usuários (cad_usuario / ExportarAbrangencia).
 *
 * Retorno raso: codUsu, usuPlt (login plataforma), codEmp, codFil — sem nome/email.
 */
class SeniorUsuarioClient
{
    ...
    {
        $service = $this->config['usuario_service'] ?? 'sapiens_Synccom_senior_g5_co_ger_cad_usuario';
...
XML;
    ...
    public function parseResponse(string $xml): array
    {
        ...
        $usuarios = $this->extractUsuarios($flat);

        // ExportarAbrangencia costuma retornar <usuario> sem tipoRetorno quando OK.
        $sucesso = !$temErroExec && !$temErroNegocio && (
            (string) ($tipoRetorno ?? '') === '1'
            || ((string) ($tipoRetorno ?? '') === '0' && $msgSucesso)
            || ($tipoRetorno === null && $usuarios !== [])
        );
        if (!$sucesso) {
    ...
    private function extractUsuarios(array $flat): array
    {
        $raw = $flat['usuario'] ?? $flat['usuarios'] ?? $flat['abrangencia'] ?? [];
        if (!is_array($raw) || $raw === []) {
            return [];
        }
        if (!isset($raw[0]) && !isset($raw['codUsu']) && !isset($raw['usuPlt'])) {
            return [];
        }
        ...
        return array_values(array_filter(array_map(function ($u) {
            ...
            return [
                'codUsu' => $codUsu,
                'usuPlt' => $this->scalarString($u['usuPlt'] ?? null),
                'codEmp' => isset($u['empresa']['codEmp']) ? (int) $u['empresa']['codEmp'] : (isset($u['codEmp']) ? (int) $u['codEmp'] : null),
                'codFil' => isset($u['abrangencia']['codFil']) ? null : (isset($u['codFil']) ? (int) $u['codFil'] : null),
            ];
        }, $raw)));
    }
    ...
    {
        $markers = ['usuario', 'usuarios', 'abrangencia', 'tipoRetorno', 'mensagemRetorno', 'erroExecucao', 'erros'];
    ...
    /** @return array<int, array<string, mixed>> */
    public function exportarAbrangencia(int $codEmp, int $codFil = 1): array
    {
        $params = [
            'codEmp' => $codEmp,
            'codFil' => $codFil,
    ...
    private function callOnce(array $params): array
    {
        ...
        while (true) {
            try {
                ...
                return $this->parseResponse($res->body())['usuarios'];
