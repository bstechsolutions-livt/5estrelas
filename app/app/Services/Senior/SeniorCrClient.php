...
/**
 * Cliente SOAP read-only para Contas a Receber (ConsultarTitulosAbertosCR).
 *
 * CONTRATO REAL validado em produção (11/07/2026):
 *  - codCli é OBRIGATÓRIO (análogo ao codFor em CP).
 *  - Params mínimos: codEmp + codCli + retRat (+ janela vctIni/vctFim em dd/MM/yyyy).
 *  - tipoRetorno=1 = sucesso.
 */
class SeniorCrClient
{
    ...
    {
        return rtrim($this->endpointBase(), '/') . '/' . $this->config['cr_service'];
    }
