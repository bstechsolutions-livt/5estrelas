 ...
 *
 * Autenticação oficial: headers `cnpjsh` + `tokensh`.
 * Pedaço 1: GET /payer/list (handshake).
 * Pedaço 2.1: GET/POST/PUT /payer (pagador / Extrato Open Finance).
 *
 * Não registra tokensh, token do pagador, CNPJ da SH nem payload bruto.
 */
class TecnoSpeedClient
{
     ...
     * @throws TecnoSpeedException
     */
    private function throwFromResponse(Response $response, bool $treat422AsAuth): never
    {
        $status = $response->status();
        ...
        if ($status === 422) {
            ...
            throw new TecnoSpeedException(
                'A TecnoSpeed recusou os dados do pagador. Verifique o cadastro da empresa/filial e tente novamente.',
                TecnoSpeedException::KIND_VALIDATION,
                $status,
            );
        }
